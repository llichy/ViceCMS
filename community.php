...
define('CSS', 'community');
define('JS', 'community');
define('TITLE', 'Communnauté');
define('DESCRIPTION', 'Voici la page communauté de '.sitename.'!');
...
$query = $db->connect()->prepare('SELECT amount,type,users.look,users.username FROM users_currency INNER JOIN users ON users_currency.user_id = users.id and users_currency.type = ? ORDER BY users_currency.amount DESC LIMIT 3');              
                $query->execute([5]);
